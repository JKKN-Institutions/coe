import type { CourseTemp, CourseTempFormData } from '@/types/courses-temp'

export async function fetchCourses(): Promise<CourseTemp[]> {
...
export async function createCourse(data: Partial<CourseTempFormData>): Promise<CourseTemp> {
	const payload = {
...
export async function updateCourse(id: string, data: Partial<CourseTempFormData>): Promise<CourseTemp> {
	const payload = {
		institution_code: data.institution_code,
		regulation_code: data.regulation_code,
		offering_department_code: data.offering_department_code || null,
	...
	const response = await fetch(`/api/master/courses-temp/${id}`, {
		method: 'PUT',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify(payload)
	})
...
export async function fetchDropdownData() {
	try {
		const [instRes, deptRes, regRes, boardRes] = await Promise.all([
			...
			fetch('/api/master/board').catch(() => null),
		])
		...
		const boards = boardRes && boardRes.ok
			? (await boardRes.json()).filter((b: any) => b?.board_code).map((b: any) => ({ id: b.id, board_code: b.board_code }))
			: []
...
export async function downloadTemplate() {
	const response = await fetch('/api/master/courses-temp/template')
	...
	a.href = url
	a.download = `Course_Temp_Template_${new Date().toISOString().split('T')[0]}.xlsx`
