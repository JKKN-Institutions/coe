...
export default function DummyNumbersPage() {
	...
	const fetchSessions = async () => {
		try {
			const res = await fetch(`/api/examination-sessions?institutions_id=${selectedInstitution}`)
			if (res.ok) {
				const data = await res.json()
				setSessions(data)
			}
	...
	const fetchFilterOptions = async () => {
		try {
			...
			const boardsRes = await fetch(`/api/boards?institutions_id=${selectedInstitution}`)
			if (boardsRes.ok) {
			...
			const coursesRes = await fetch(`/api/courses?institution_code=${institutions.find(i => i.id === selectedInstitution)?.institution_code || ''}`)
			if (coursesRes.ok) {
			...
			// Fetch programs
			const programsRes = await fetch(`/api/program?institution_code=${institutions.find(i => i.id === selectedInstitution)?.institution_code || ''}`)
			if (programsRes.ok) {
	...
	const fetchDummyNumbers = async () => {
		try {
			setLoading(true)
			let url = `/api/dummy-numbers?institutions_id=${selectedInstitution}&examination_session_id=${selectedSession}`
	...
	const handleGenerate = async () => {
		...
		try {
			...
			const res = await fetch('/api/dummy-numbers/generate', {
				method: 'POST',
	...
	const handleDeleteAll = async () => {
		...
		try {
			...
			const res = await fetch(
				`/api/dummy-numbers?institutions_id=${selectedInstitution}&examination_session_id=${selectedSession}`,
				{ method: 'DELETE' }
			)
