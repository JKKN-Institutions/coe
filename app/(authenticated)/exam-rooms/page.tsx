...
export default function ExamRoomsPage() {
	...
	const handleSave = async () => {
		...
		try {
			...
			const payload = {
				...formData,
				room_order: Number(formData.room_order),
				seating_capacity: Number(formData.seating_capacity),
				exam_capacity: Number(formData.exam_capacity),
			...
			if (!response.ok) {
				...
				throw new Error(errorData.error || 'Failed to save exam room')
			}
	...
	const handleDelete = async () => {
		...
		try {
			const response = await fetch(`/api/exam-rooms?id=${itemToDelete.id}`, {
				method: 'DELETE',
			})

			if (!response.ok) throw new Error('Failed to delete exam room')
