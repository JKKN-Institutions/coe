...
export default function GradeSystemPage() {
	...
	const fetchGradeSystems = async () => {
		try {
			setLoading(true)
			const response = await fetch('/api/grade-system')
			if (!response.ok) {
	...
	const validate = () => {
		...
		if (!formData.grade_system_code.trim()) e.grade_system_code = "Required"
		if (!formData.grade_id.trim()) e.grade_id = "Required"
		if (!formData.regulation_id.trim()) e.regulation_id = "Required"
		if (!formData.min_mark.trim()) e.min_mark = "Required"
		if (!formData.max_mark.trim()) e.max_mark = "Required"
	...
	const remove = async (id: string) => {
		try {
			...
			const response = await fetch(`/api/grade-system?id=${id}`, {
				method: 'DELETE',
			})
