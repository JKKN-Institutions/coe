...
export default function AttendanceReportsPage() {
	...
	const fetchCourses = async () => {
		...
		try {
			...
			const res = await fetch(
				`/api/exam-attendance/dropdowns?type=courses&institution_id=${selectedInstitutionId}&session_id=${selectedSessionId}&program_code=${selectedProgramCode}&exam_date=${selectedExamDate}&session_type=${selectedSessionType}`
			)
			if (res.ok) {
				const data = await res.json()
				setCourses(data)
	...
	const handleGenerateStudentSheet = async () => {
		...
		try {
			...
			const response = await fetch(`/api/exam-attendance/student-sheet?${params.toString()}`)
	...
	const handleGenerateSummary = async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/exam-attendance/report?institution_id=${selectedInstitutionId}&session_code=${session.session_code}`
			)
