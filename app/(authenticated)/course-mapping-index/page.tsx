...
export default function CourseMappingIndexPage() {
	...
	const filteredGroups = groups.filter(group => {
		...
		return (
			...
			group.batch_code?.toLowerCase().includes(searchLower) ||
			group.batch_name?.toLowerCase().includes(searchLower) ||
			group.batch_year?.toLowerCase().includes(searchLower)
		)
	})
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-muted-foreground">
											View and manage course mappings by program, regulation, and batch
										</p>
									</div>
								</div>
								<div className="flex gap-2">
									<Button variant="outline" asChild>
										<Link href="/course-mapping">
								...
								<Table>
									<TableHeader>
										<TableRow className="bg-muted/50">
											<TableHead className="w-[300px] font-semibold">Degree-Branch</TableHead>
											<TableHead className="font-semibold">Regulation</TableHead>
											<TableHead className="font-semibold">Batch</TableHead>
											<TableHead className="text-center font-semibold">Total Courses</TableHead>
									...
									<TableBody>
										{loading ? (
											...
											currentGroups.map((group, index) => (
												<TableRow key={`${group.institution_code}-${group.program_code}-${group.regulation_code}-${group.batch_code}`} className="hover:bg-muted/50">
													<TableCell>
														<div className="flex flex-col">
															<span className="font-medium">
																{group.program_name || group.program_code}
															</span>
													...
													<TableCell>
														<div className="flex flex-col">
															<span className="text-sm">{group.batch_code}</span>
															{group.batch_year && (
																<span className="text-xs text-muted-foreground">{group.batch_year}</span>
															)}
														</div>
													</TableCell>
													...
													<TableCell className="text-center">
														<div className="flex items-center justify-center gap-2">
															<Button
																...
																className="h-8"
															>
																<Link href={`/course-mapping/edit?institution=${group.institution_code}&program=${group.program_code}&regulation=${group.regulation_code}&batch=${group.batch_code}`}>
															...
															<Button
																...
																onClick={async () => {
																	try {
																		...
																		const response = await fetch(`/api/course-mapping/report?institution_code=${group.institution_code}&program_code=${group.program_code}&batch_code=${group.batch_code}&regulation_code=${group.regulation_code}`)
