...
export default function AddPermissionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else setError('Failed to create permission')
    } catch (err) { setError('Failed to create permission') } finally { setLoading(false) }
  }
          ...
          <div className="flex items-center gap-2">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage className="text-xs">Add Permission</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    <Button type="submit" disabled={loading} size="sm" className="h-8 text-xs">
                      {loading ? 'Saving…' : (<><Save className="h-3 w-3 mr-1"/> Create Permission</>)}
                    </Button>
                  </div>
                </form>
              </CardContent>
