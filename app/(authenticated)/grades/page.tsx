...
export default function GradesPage() {
	...
	const save = async () => {
		...
		try {
			...
			if (editing) {
				const res = await fetch('/api/grades', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editing.id, ...payload }) })
				if (!res.ok) {
		...
		if (institutions.length === 0 || regulations.length === 0) {
			...
			try {
				...
				if (regulations.length === 0) {
					const resReg = await fetch('/api/regulations')
					if (resReg.ok) {
