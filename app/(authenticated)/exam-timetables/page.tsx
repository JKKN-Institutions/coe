...
export default function ExamTimetablesListPage() {
	...
	const remove = async (id: string) => {
		try {
			...
			const response = await fetch(`/api/exam-timetables?id=${id}`, {
				method: 'DELETE',
			})
	...
	const processUpload = async (rows: any[]) => {
		...
		for (let i = 0; i < rows.length; i++) {
			...
			try {
				const institutionResponse = await fetch(`/api/institutions?code=${institution_code}`)
				...
				const sessionResponse = await fetch(`/api/examination-sessions`)
