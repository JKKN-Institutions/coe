...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!regulation_year || !regulation_code || minimum_attendance === undefined) {
      return NextResponse.json({ 
        error: 'Missing required fields: regulation_year, regulation_code, and minimum_attendance are required' 
      }, { status: 400 })
    }
