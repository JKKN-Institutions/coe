...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const mapped = data ? {
      ...
      dummy_number_not_required: data.dummy_number_not_required,
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const data: Record<string, unknown> = {
      ...
      ...(input.dummy_number_not_required !== undefined && { dummy_number_not_required: Boolean(input.dummy_number_not_required) }),
