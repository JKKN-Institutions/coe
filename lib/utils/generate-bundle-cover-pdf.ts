 ...
 * Multiple bundles are created automatically if needed
 */
export function generateBundleCoverPDF(data: BundleCoverData): string {
	// Split students into bundles of 60
	const bundles: StudentAttendance[][] = []
	for (let i = 0; i < data.students.length; i += STUDENTS_PER_BUNDLE) {
		bundles.push(data.students.slice(i, i + STUDENTS_PER_BUNDLE))
	}

	// Create a single PDF document for all bundles
	const doc = new jsPDF('landscape', 'mm', 'a4')
	const pageWidth = doc.internal.pageSize.getWidth()
	const pageHeight = doc.internal.pageSize.getHeight()
	const margin = 12.7 // 0.5 inch in mm

	bundles.forEach((bundleStudents, bundleIndex) => {
		// Add new page for each bundle (except the first one)
		if (bundleIndex > 0) {
			doc.addPage()
		}
		...
		doc.setFont('times', 'normal')
		doc.text(`: ${data.program_code} - ${data.program_name} `, leftColX + labelWidth, currentY)
		...
		// Handle long subject names with text wrapping
		...
		doc.setFont('times', 'normal')
		const sessionFull = data.session === 'FN' ? 'FORENOON' : 'AFTERNOON'
		doc.text(`: ${data.exam_date} - ${sessionFull}`, leftColX + labelWidth, currentY)

		// Bundle Number (right side, same row, right-aligned)
		doc.setFont('times', 'bold')
		doc.setFontSize(11)
		const bundleText = 'Bundle No.'
		const bundleNumberText = `(Bundle ${bundleIndex + 1} of ${bundles.length})`
		...
		// Add bundle number text next to box
		doc.setFont('times', 'normal')
		doc.text(bundleNumberText, boxX + boxSize + 5, currentY)

		currentY += 8  // Reduced from 12 to 8 (no line space)
		...
		// Fill DOWN first (column-wise), then ACROSS
		const tableData: any[][] = []

		// Create 10 rows (ROWS_PER_COLUMN)
		for (let row = 0; row < ROWS_PER_COLUMN; row++) {
			const rowData: string[] = []

			// For each of the 6 columns
			for (let col = 0; col < COLUMNS_PER_ROW; col++) {
				// Calculate student index: fill column 1 (rows 0-9), then column 2 (rows 10-19), etc.
		...
		autoTable(doc, {
			...
			columnStyles: {
				// Register No columns (6 columns, narrower width)
		...
		// Calculate layout widths for single row
		...
		// Chief Superintendent Signature (left section) - CENTER ALIGNED & BOLD with space for signature
		doc.setFont('times', 'bold')
		doc.setFontSize(12)
		...
		// Notes (center section) - ALL IN SAME ROW
		doc.setFont('times', 'bold')
		doc.setFontSize(6.5)
		const notesText = 'Notes: '
		doc.text(notesText, notesX, bottomY)

		// Notes list (no line spacing)
		doc.setFont('times', 'normal')
		doc.setFontSize(10)
		...
		// Additional Superintendent Signature (right section) - CENTER ALIGNED & BOLD with space for signature
		doc.setFont('times', 'bold')
		doc.setFontSize(12)
		const addSigCenterX = addSigX + (addSigWidth / 2)
		doc.text('Signature of the Additional', addSigCenterX, bottomY, { align: 'center' })
	...
	const sessionName = data.session_name || data.exam_date
	const fileName = `Bundle_Cover_${data.subject_code}_${data.exam_date.replace(/\//g, '-')}_${data.session}_${new Date().toISOString().split('T')[0]}.pdf`
	doc.save(fileName)
